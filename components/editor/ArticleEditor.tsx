...
export function RichEditor({ value, onChange, hasError }: { value: string; onChange: (html: string) => void; hasError?: boolean }) {
    ...
    <>
      ...
      <div style={{ borderRadius: 12, border: hasError ? "1.5px solid #f87171" : "1.5px solid #e5e5e5", overflow: "hidden", background: "#fff" }}>
        ...
        <div
          ...
          style={{ minHeight: 480, padding: "18px 20px", fontSize: 15, lineHeight: 1.8, color: "#0b0b0c", outline: "none", overflowY: "auto" }}
        />
      </div>
...
export function ImageDropZone({ imageFile, imagePreview, onFileChange, onClear }: { imageFile: File | null; imagePreview: string | null; onFileChange: (f: File) => void; onClear: () => void }) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [drag, setDrag] = useState(false);

  const handle = useCallback((file: File) => { if (file.type.startsWith("image/")) onFileChange(file); }, [onFileChange]);
  const onDrop = useCallback((e: React.DragEvent) => { e.preventDefault(); setDrag(false); const f = e.dataTransfer.files?.[0]; if (f) handle(f); }, [handle]);
