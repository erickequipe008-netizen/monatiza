...
export default function MagazineCard({ m }: { m: MagazinePublic }) {
 ...
 <Link
 href={`/revista/${m.slug}`}
 className="relative block aspect-[3/4] overflow-hidden rounded-2xl bg-zinc-100 shadow-sm ring-1 ring-black/[0.06] transition duration-300 group-hover:-translate-y-1 group-hover:shadow-xl"
 >
 {m.cover_url ? (
 ...
 {m.subtitle && (
 <p className="mt-1 line-clamp-2 text-[13px] leading-snug text-zinc-500">{m.subtitle}</p>
 )}
