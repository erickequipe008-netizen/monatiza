  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-8">
        ...
        <Link
          href="/editorial/revista/producao"
          className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium text-white transition-all duration-150 hover:opacity-90 active:scale-95 mt-1"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {kpis.map((kpi) => (
          <div key={kpi.label} className="bg-white rounded-2xl border border-[#E5E7EB] px-5 py-4 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              <span className="w-2 h-2 rounded-full" style={{ background: kpi.dot }} />
              <p className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wide">{kpi.label}</p>
            </div>
            <p className="text-3xl font-bold" style={{ color: kpi.text }}>{kpi.value}</p>
          </div>
        ))}
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[#F3F4F6]">
                {pedidos?.map((pedido: any) => (
                  <tr key={pedido.id} className="hover:bg-[#F5F6FF] transition-colors duration-100">
