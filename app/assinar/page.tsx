...
function Rule({ ok, label }: { ok: boolean; label: string }) {
  return (
    <li className="flex items-center gap-2.5 text-[13.5px]">
      <span
        className={`flex h-5 w-5 items-center justify-center rounded-full transition ${ok ? "text-white" : "border border-zinc-300 text-transparent"}`}
        style={ok ? { backgroundColor: RED } : undefined}
      >
...
const inputCls =
  "h-13 w-full rounded-2xl border border-zinc-200 bg-zinc-50 px-4 text-[15px] text-zinc-900 placeholder:text-zinc-400 outline-none transition focus:border-[#E0263B] focus:bg-white focus:ring-2 focus:ring-[#E0263B]/15";

function AssinarForm() {
  const params = useSearchParams();
  const plano: "mensal" | "anual" = params.get("plano") === "anual" ? "anual" : "mensal";
  const paymentUrl = `/assinar/pagamento?plano=${plano}`;
  ...
  // Já logado? Vai direto para o pagamento — sem passar por cadastro.
  useEffect(() => {
    (async () => {
      const {
        data: { session },
  ...
  const canSubmit = name.trim().length >= 2 && emailOk && rules.len && rules.upper && rules.lower;
  ...
  async function signupGoogle() {
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}${paymentUrl}` },
    });
  }
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#08080b] p-4">
      <div className="pointer-events-none absolute -left-40 top-0 h-[460px] w-[560px] rounded-full bg-[#E0263B]/15 blur-[120px]" />
      <div className="pointer-events-none absolute -right-40 bottom-0 h-[460px] w-[560px] rounded-full bg-[#E0263B]/10 blur-[120px]" />

      <div className="pro-pop relative w-full max-w-[480px] rounded-3xl bg-white p-7 text-zinc-900 shadow-2xl sm:p-9">
        <div className="mb-6 flex items-center justify-between">
          <Link href="/assinantes" aria-label="Fechar" className="text-zinc-500 transition hover:text-zinc-900">
            <X size={22} />
          </Link>
          <span className="text-[12px] font-bold text-zinc-400">Assinatura</span>
        </div>

        <h1 className="text-[26px] font-black tracking-tight sm:text-[30px]">Crie sua conta e assine</h1>
        ...
        <form onSubmit={handleSignup} className="mt-6 space-y-5">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Seu nome" required className={`${inputCls} mt-2`} />
          </div>

          <div>
            <label className="text-[15px] font-bold">E-mail</label>
            <p className="mb-2 text-[13px] text-zinc-500">Usamos seu e-mail para acesso, suporte e recuperação da conta.</p>
          ...
          <button
            ...
            className="flex h-13 w-full items-center justify-center gap-2 rounded-full text-[15px] font-bold text-white transition hover:brightness-110 disabled:opacity-40"
        ...
        <p className="mt-5 text-center text-sm text-zinc-500">
          Já tem conta?{" "}
          <Link href={`/painel/login?next=${encodeURIComponent(paymentUrl)}`} className="font-bold" style={{ color: RED }}>
            Entrar
          </Link>
        </p>
        <p className="mt-4 text-center text-[12px] leading-relaxed text-zinc-400">
          ...
          <Link href="/privacy" className="underline">Política de Privacidade</Link>. O pagamento é no ambiente seguro em seguida.
        </p>
      </div>
    </main>
  );
