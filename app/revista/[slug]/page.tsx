...
export default async function MagazineDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 md:px-6 md:py-12">
      ...
      <Link
        href="/revista"
        className="mb-6 inline-flex items-center gap-1.5 text-[13px] font-semibold text-zinc-500 transition hover:text-zinc-900 dark:hover:text-white"
      >
      ...
      <div className="grid gap-8 md:grid-cols-[minmax(0,340px)_1fr] md:gap-12">
        ...
        <div className="mx-auto w-full max-w-[320px] md:sticky md:top-8 md:self-start">
          <div className="relative aspect-[3/4] overflow-hidden rounded-2xl bg-zinc-100 shadow-xl ring-1 ring-black/[0.06] dark:bg-zinc-900 dark:ring-white/10">
            {m.cover_url ? (
        ...
        <div>
          ...
          <h1 className="mt-2 text-[30px] font-black leading-tight tracking-tight text-zinc-900 md:text-[40px] dark:text-white">
            {m.title}
          </h1>
          {m.subtitle && <p className="mt-2 text-[16px] leading-relaxed text-zinc-500 dark:text-zinc-400">{m.subtitle}</p>}

          <div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-1 text-[13px] text-zinc-500 dark:text-zinc-400">
            {m.edition && <span>Edição {m.edition}</span>}
            {m.edition && <span className="text-zinc-300 dark:text-zinc-600">·</span>}
            <span>Publicado em {date}</span>
          </div>

          {m.description && (
            <div className="mt-6 whitespace-pre-line text-[15.5px] leading-relaxed text-zinc-700 dark:text-zinc-300">
              {m.description}
            </div>
          )}

          {/* Compra */}
          <div className="mt-8 rounded-2xl border border-black/[0.08] bg-white p-5 dark:border-white/10 dark:bg-zinc-900/60">
            <div className="flex items-end justify-between">
              <div>
                <p className="text-[12px] font-semibold uppercase tracking-wide text-zinc-400">Edição digital · PDF</p>
                <p className="text-[30px] font-black tracking-tight text-zinc-900 dark:text-white">{formatPrice(m.price)}</p>
              </div>
            </div>
            <div className="mt-4">
              <BuyButton magazineId={m.id} label="Comprar agora" size="lg" />
            </div>
            <ul className="mt-4 space-y-2 text-[13px] text-zinc-500 dark:text-zinc-400">
