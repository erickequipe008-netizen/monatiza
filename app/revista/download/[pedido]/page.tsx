...
export default async function DownloadPage({ params }: { params: Promise<{ pedido: string }> }) {
  ...
  if (!r.paid) {
    return (
      <Shell>
        ...
        <h1 className="mt-4 text-[22px] font-black tracking-tight text-zinc-900 dark:text-white">Pagamento não localizado</h1>
        ...
        <Link href="/revista" className="mt-6 inline-block rounded-full bg-zinc-900 px-6 py-3 text-[14px] font-bold text-white dark:bg-white dark:text-black">
          Voltar à loja
        </Link>
      </Shell>
    );
  ...
  return (
    <Shell>
      ...
      <h1 className="mt-4 text-[24px] font-black tracking-tight text-zinc-900 dark:text-white">Pagamento aprovado! 🎉</h1>
      ...
      <div className="mt-6 w-full rounded-2xl border border-black/[0.08] bg-zinc-50 p-4 text-left dark:border-white/10 dark:bg-zinc-900/60">
        <p className="text-[15px] font-extrabold text-zinc-900 dark:text-white">{title}</p>
